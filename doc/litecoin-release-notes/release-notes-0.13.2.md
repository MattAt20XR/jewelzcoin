JewelzCoin Core version 0.13.2 is now available from:

  <https://download.jewelzcoin.org/jewelzcoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a jewelzcoin
...
ECDSA signatures inside JewelzCoin transactions now use validation using
...
Previous versions of JewelzCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of JewelzCoin Core was to raise the default minimum
relay fee.

JewelzCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
JewelzCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running jewelzcoind without having to do any manual configuration.
...
JewelzCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. JewelzCoin Core will relay transactions with
...
entirely in JewelzCoin Core 0.13, and it is at this time undecided whether the
...
JewelzCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
JewelzCoin Core automatically creates a hidden service to listen on, without
manual configuration. JewelzCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if JewelzCoin Core is listening, and
...
JewelzCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals JewelzCoin
Core to use floating fees. By default, JewelzCoin Core will use floating
fees.
...
(default: `0.0002` JLZ/kB).

At all times, JewelzCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) JLZ.
Furthermore, JewelzCoin Core will never create transactions paying less than
...
`jewelzcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /jewelzcoinrpc>
        ...
        # optional bypass jewelzcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< jewelzcoinrpc:<password>
    </Location>
...
Restarting jewelzcoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in jewelzcoin.conf
...
jewelzcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/jewelzcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The JewelzCoin Core code base has
...
- `jewelzcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `jewelzcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the JewelzCoin scripting system that in combination with
...
JewelzCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of JewelzCoin Core.
...
The two phases are distinct in the JewelzCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming JewelzCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for JewelzCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each JewelzCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of jewelzcoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of jewelzcoins being spent is exactly
...
- **Increased security for multisig:** JewelzCoin addresses (both P2PKH addresses
  ...
  believed to be provided by JewelzCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of JewelzCoin's current ledger (called the UTXO set),
  ...
  note that JewelzCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  JewelzCoin users to individually opt-in to almost any change in the JewelzCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and JewelzCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the JewelzCoin
client to JewelzCoin Core 0.13.2 to signal for segwit support. When a
...
Since JewelzCoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `jewelzcoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `jewelzcoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
