...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete JewelzCoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no JewelzCoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old JewelzCoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected JewelzCoin 0.7 datadir."
    ;;
  3)
    echo "Detected JewelzCoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected JewelzCoin 0.8 datadir."
    ;;
esac
